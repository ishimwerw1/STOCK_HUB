...
export default function Warehouses() {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-white to-emerald-50/30 rounded-2xl p-5 md:p-8 border border-emerald-100/50 shadow-lg shadow-emerald-500/5 animate-slide-in">
        <div className="flex items-center gap-3 mb-6">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${editingId ? 'bg-amber-100 text-amber-600 animate-shake' : 'bg-emerald-100 text-emerald-600'}`}>
            {editingId ? '✏️' : '🏭'}
          </div>
          <div>
        ...
        {loading ? <Loading text="Loading warehouses" /> : warehouses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {warehouses.map((w, i) => (
              <div
                key={w._id}
                className="group bg-white rounded-2xl p-5 border border-slate-100 hover:border-emerald-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 animate-scale-in"
                style={{ animationDelay: `${i * 0.05}s` }}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500/10 to-teal-500/10 flex items-center justify-center text-xl group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                      🏭
                    </div>
                    <div>
                ...
                <div className="bg-gradient-to-r from-emerald-50/50 to-teal-50/50 rounded-xl p-4 mb-4 group-hover:from-emerald-100/50 group-hover:to-teal-100/50 transition-all duration-300">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(w)} className="flex-1 py-2 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 text-sm font-medium transition-all duration-200 flex items-center justify-center gap-1.5 border border-blue-200 hover:shadow-md active:scale-[0.97]">
                    <span>✏️</span> Edit
                  </button>
                  <button onClick={() => handleDelete(w._id)} className="flex-1 py-2 rounded-xl bg-red-50 text-red-700 hover:bg-red-100 text-sm font-medium transition-all duration-200 flex items-center justify-center gap-1.5 border border-red-200 hover:shadow-md active:scale-[0.97]">
                    <span>🗑️</span> Delete
                  </button>
                </div>
              </div>
