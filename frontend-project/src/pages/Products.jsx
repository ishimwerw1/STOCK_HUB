...
export default function Products() {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-white to-blue-50/30 rounded-2xl p-5 md:p-8 border border-blue-100/50 shadow-lg shadow-blue-500/5 animate-slide-in">
        <div className="flex items-center gap-3 mb-6">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${editingId ? 'bg-amber-100 text-amber-600 animate-shake' : 'bg-blue-100 text-blue-600'}`}>
            {editingId ? '✏️' : '📦'}
          </div>
          <div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            {[
              ...
              { name: 'unitPrice', placeholder: 'Unit Price (RWF)', type: 'number', min: 0, step: '1' },
              ...
              <input
                ...
                className="px-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all duration-300 hover:border-blue-300 hover:shadow-sm shadow-sm"
                required
              />
            ))}
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            {editingId && (
              <button type="button" onClick={resetForm} className="w-full sm:px-8 py-3 rounded-xl border-2 border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all font-semibold">
                Cancel
              </button>
            )}
          </div>
      ...
      <div className="animate-slide-in" style={{ animationDelay: '0.1s' }}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
          ...
          <div className="flex gap-1">
            {['all', 'low'].map((f) => (
              <span key={f} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors cursor-pointer">{f === 'all' ? 'All' : 'Low Stock'}</span>
            ))}
          </div>
        </div>
        ...
        {loading ? <Loading text="Loading products" /> : products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {products.map((p, i) => (
              <div
                key={p._id}
                className="group bg-white rounded-2xl p-5 border border-slate-100 hover:border-blue-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 animate-scale-in"
                style={{ animationDelay: `${i * 0.05}s` }}
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500/10 to-purple-500/10 flex items-center justify-center text-lg shrink-0 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                      📦
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-semibold text-slate-900 group-hover:text-blue-600 transition-colors truncate">{p.productName}</h3>
                      <span className="text-xs text-slate-400 font-mono">{p.productCode}</span>
                    </div>
                  </div>
                  <span className="px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 text-xs font-medium shrink-0 ml-2">{p.category}</span>
                </div>

                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-slate-50 rounded-xl p-3 group-hover:bg-slate-100/50 transition-colors">
                    <p className="text-xs text-slate-400">Stock</p>
                    <p className={`text-lg font-bold ${p.quantityInStock < 10 ? 'text-red-500 animate-pulse' : 'text-slate-900'}`}>
                      {p.quantityInStock}
                  ...
                  <div className="bg-slate-50 rounded-xl p-3 group-hover:bg-slate-100/50 transition-colors">
                    ...
                    <p className="text-lg font-bold text-emerald-600">{formatRWF(p.unitPrice)}</p>
                  </div>
                  <div className="bg-slate-50 rounded-xl p-3 group-hover:bg-slate-100/50 transition-colors">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(p)} className="flex-1 py-2 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 text-sm font-medium transition-all duration-200 flex items-center justify-center gap-1.5 border border-blue-200 hover:shadow-md active:scale-[0.97]">
                    <span>✏️</span> Edit
                  </button>
                  <button onClick={() => handleDelete(p._id)} className="flex-1 py-2 rounded-xl bg-red-50 text-red-700 hover:bg-red-100 text-sm font-medium transition-all duration-200 flex items-center justify-center gap-1.5 border border-red-200 hover:shadow-md active:scale-[0.97]">
                    <span>🗑️</span> Delete
                  </button>
                </div>
              </div>
